...
def plot_feature_importance(
    ...
    colors = [
        lob_color if idx in lob_feat else default_color
        for idx in top_feats.index
    ]
    ...
    ax.invert_yaxis()
    ax.set_xlabel(f"{importance_type} mean feature importance", fontsize=tick_label_fontsize)
    ax.set_title(f"{model_name} {importance_type} feature importance — {target_name}", fontsize=title_fontsize)
    ...
    ax.legend(handles=[lob_patch, mat_patch], fontsize=tick_label_fontsize, loc="lower right")
